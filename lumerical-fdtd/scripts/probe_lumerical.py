...
def default_roots() -> list[Path]:
    ...
    return [
        ...
        Path("/opt/Lumerical"),
        Path("/opt/ansys_inc"),
        Path("/usr/local/lumerical"),
        Path("/usr/local/Lumerical"),
    ]


def fdtd_executable_name() -> str:
    return "fdtd-solutions.exe" if host_os() == "windows" else "fdtd-solutions"
...
def discover_cli(explicit: str | None) -> Path | None:
    ...
    for _version_dir, install_root in lumerical_version_dirs():
        candidates.append(install_root / "bin" / fdtd_executable_name())
    executable_name = fdtd_executable_name()
    path_value = os.environ.get("PATH", "")
    for entry in path_value.split(os.pathsep):
        if entry:
            candidates.append(Path(entry) / executable_name)
    candidates = unique_existing_order(candidates)
    for candidate in candidates:
        if candidate.exists():
            return candidate.resolve()
...
def normalize_env(api_path: Path | None, cli_path: Path | None, license_server: str) -> dict[str, str]:
    ...
    if install_root:
        prepend = [
            install_root / "bin",
            install_root / "licensingclient" / ("winx64" if host_os() == "windows" else "linux64"),
        ]
